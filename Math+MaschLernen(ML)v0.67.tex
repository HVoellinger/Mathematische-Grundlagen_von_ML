...
\begin{center}
Diese Version V0.67 (Datum: 29.9.2023) ist nicht final!!! \\
...
$
\\[0.2cm]
In der Literatur heisst $X$ auch oft {\color{blue}{Feature Matrix}}. $X$ ist eine (nxk)-Matrix. So definiert sind die Zeilenvektoren der Matrix $X$ die transponierten Vektoren $ \mathbf{\widetilde{x^{(i)}}} $ mit $ 1 \lneq i \lneq n $  wobei n = Anzahl der Trainingstupel. Schreiben wir die einzelnen Komponenten (Skalare) der (nxk)-Matrix $X$ aus,  so ergibt sich: 
\\[0.3cm]
\hspace*{4.8cm}
$ X := \left(
  \begin{array}{c}
