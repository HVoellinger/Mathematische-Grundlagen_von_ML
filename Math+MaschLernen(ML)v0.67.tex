...
\begin{center}
Diese Version V0.67 (Datum: 30.9.2023) ist nicht final!!! \\
...
$     
\\[0.4cm]
Angenommen die Matrix $X^\top \cdot X$ it invertierbar, so lässt sich obige Matrix umschreiben zu:
\\[0.2cm]
\hspace*{4.8cm}
...
Diese Berechnungen sind als (Übung-5.9) durchzuführen. Eine komplette Lösung dazu finden sie auch im Abschnitt "Lösungshinweise zu den Übungen Kapitel 5".\\
%
\\[0.3cm]    
...
\\[0.2cm]
Der Herleitung der Normalform ist dabei analog wie mit dem "Bias-Trick". Wir erhalten für die Faktoren der {\color{blue}{Normalform}}:
\\[0.4cm]
\hspace*{-0.3cm}
$ X^\top \cdot X := \left(
  \begin{array}{c}
...
Einsetzen dieser beiden Faktoren in die Formeln von (Th-5.5) ergibt die Ergebnisse des Theorem (Th-5.2) fü die simple lineare Regression. \\
\textbf{Hinweis:} Die konkrete Berechnung  sind als (Übung-5.8) durchzuführen. Eine komplette Lösung dazu finden sie auch im Abschnitt "Lösungshinweise zu den Übungen Kapitel 5".\\
%
q.e.d. \\    
...
%
Beweise die Aussage von Anmerkung (A-5.2) und lerne dabei die "Allgemeinen Berechnungen zu LSF-Verfahren (ohne Bias-Trick)" besser kennen. Beachte die Hinweise. 
