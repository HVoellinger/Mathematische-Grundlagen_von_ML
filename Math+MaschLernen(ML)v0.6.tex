...
\begin{center}
Diese Version V0.6 (Datum: 28.8.2023) ist nicht final!!! \\
...
Um eine Animation einer 3-Means-Konvergenz mit 15 Iterationen zu sehen können sie auf folgenden Link aufrufen:\\
...
\begin{center}
\hspace*{-1.0cm}   
\includegraphics[width=0.8\textwidth]{K-Means-Python-Bild1}
\hspace*{-1.0cm}   
\includegraphics[width=0.8\textwidth]{K-Means-Python-Bild2}
\hspace*{-1.0cm} 
\includegraphics[width=0.8\textwidth]{K-Means-Python-Bild3}
\hspace*{-1.0cm} 
\includegraphics[width=0.8\textwidth]{K-Means-Python-Bild4}
\end{center}
...
Siehe Lösungen in meinem Github:\\[0.2cm] 

\url{https://github.com/HVoellinger/Mathematische-Grundlagen_von_ML/blob/main/Quellen_Math-ML/k-means-HVoe.pdf} \\


\url{https://github.com/HVoellinger/Mathematische-Grundlagen_von_ML/blob/main/Quellen_Math-ML/k-means-HVoe.tex}\\
...
\subsection{Mathematische Grundlagen GINI- und Entropie-Verfahren }
...
Der Informationstheoretische Verständnis des Begriffes \textbf     {Entropie} geht auf [Claude Elwood Shannon] zurück und existiert seit etwa 1948. In diesem Jahr veröffentlichte Shannon seine fundamentale Arbeit "A Mathematical Theory of Communication"\\[0.1cm]
...
\begin{center}
$ \mathrm{H_n} = -\sum_{w \in Z^n} p_w \log_2 p_w $
\end{center}
...
\url{https://de.wikipedia.org/wiki/Bedingte_Entropie#Blockentropie}.\\[0.1cm]
...
\textbf{Anmerkung:} Wir sehen später das für  optimale sLR-Geraden eine andere Formel für R² auch gültig ist. Diese Formel benutzt die Kennzahl SSR.\\[0.2 cm]

Im folgenden Bild sehen wir die Kennzahlen {\color{blue}{SSE}} und {\color{red}{SST}} geometrisch mit 4 Beobachtungspunkten. Sehr gut sind die Größen der Quadrate für {\color{blue}{SSE}} und {\color{red}{SST}} zu erkennen. Man bekommt zudem auch ein guten Eindruck von den Faktor 
\begin{large}
$ \frac{\color{blue}{SSE}}{\color{red}{SST}} $\\[6.8 cm]
\end{large}

\begin{figure}[htp]
\hspace*{-2.0cm}   
\includegraphics[width=1.0\textwidth]{SSE_SST-Definition}
...
(i)  $ \quad\quad 0 \leqslant R^2 \leqslant 1\quad\quad\quad\quad $   "Begrenzung"\\[0.2cm]                                       
(ii) $ R^2 = 0 \Leftrightarrow SSE = SST \quad $                      "Minimum"\\[0.2cm]                                       
(iii) $ R^2² = 1 \Leftrightarrow SSE = 0 \quad\quad $                 "Maximum"\\[0.4cm]                                       
\textbf{Beweis:}
...
ad (iii): $SSE = 0 \Leftrightarrow \frac{SSE}{SST} = 0 \Leftrightarrow  1 + \frac{SSE}{SST} = 1 \Leftrightarrow  1 = 1 -  \frac{SSE}{SST} \Leftrightarrow  R^2 = 1 $ q.e.d. \\[0.3cm] 
...
\\[0.2cm]
\textbf{Korollar(K-4.1)}: “opt. sLR ohne Achsenabschnitt(b =0)”\\
\begin{center}
\begin{large}
Aus $ b=0 \Rightarrow m = {\dfrac{\overline{x \cdot y}}{\overline{x^2}}}$  \\[0.2cm]                                          
\end{large}
\end{center}
\textbf{Beweis:}\\
$ b = 0 \Rightarrow y = m \cdot x$\\
Da die sSLR-Gerade optimal ist, gilt die Ableitung von $R^2$ nach m = 0. Berechne nun diese. Da die Ableitung von SST konstant ist und die Gleichung später = 0 gesetzt wird, gilt: 
\begin{center}
\begin{large}
$ 0 =\frac{\partial}{\partial m} (1-\frac{SSE}{SST}) = \frac{\partial (SSE)}{\partial m} = \frac{\partial [\sum\limits_{i=1}^n [(y_i - m \cdot x_i)^2]}{\partial m} = 2 \cdot \sum(y_i \cdot x_i - m \cdot (x_i)^2) $ 
\end{large}
\end{center}
daraus folgt:
\begin{center}
\begin{large}
$ \sum(y_i \cdot x_i) =  m \cdot \sum(x_i)^2) \Rightarrow m = \frac{\sum(y_i \cdot x_i }{\sum(x_i)^2)} \Rightarrow m = \frac{\overline{x \cdot y}}{\overline{x^2}} \quad $ q.e.d.     \\[0.6cm] 
\end{large}
\end{center} 

Wir brauchen für später auch noch weitere hilfreiche Formeln über Summen und Mittelwerte. Diese werden für die Berechnung der “optimalen” Koeffizienten $b$ und $m$ gebraucht (siehe: “Least Square Fit” (LSF) Methode):\\[0.8cm]
{\color{red}{***********************************************************************\\ 
...
Sei die optimale Regression-Gerade gegeben durch $ y = b + m \cdot x $ dann ist R² eine Funktion in den Veränderlichen b und m: R² = R²(b, m). Somit muss für ein Maximum gelten (Analysis I): \\
\begin{center}
\begin{Large} 
$  dR^2 = \frac{\partial R^2}{\partial b} \cdot db + \frac{\partial R^2}{\partial m} \cdot dm = 0 \Longleftrightarrow  \frac{\partial R^2}{\partial b} = \frac{\partial R^2}{\partial m} = 0 $
\end{Large}
\end{center}

Wir rechnen als erstes  die \textbf{Ableitung nach $b$}:\\
...
\begin{center}
\begin{large} 
$ 0 = \frac{\partial SSE}{\partial b} = \frac{\partial [\sum\limits_{i=1}^n [(y_i - b -m \cdot x_i)^2]}{\partial b} = 2 \cdot \sum\ (y_i - b - m \cdot  x_i) $
\end{large}
\end{center}

Durch Benutzung der Definition des Mittelwertes und Kürzen der 2 erhält man: 
 
\begin{center}
$ 0 = n \cdot \bar{y} - n \cdot b -n \cdot m \cdot \bar{x} \Rightarrow 0 = \bar{y} - b  -m \cdot \bar{x} \Rightarrow b = \bar{y} -m \cdot \bar{x} $ (*)
\end{center}

Analog berechnen wir die \textbf{Ableitung nach $m$}:\\
 
\begin{center}
\begin{large} 
$ \frac{\partial R^2}{\partial m} = \frac{\partial [ 1 - \frac {SSE}{SST}]}{\partial m} =  \frac{\partial 1}{\partial m} - \frac{\partial [\frac{1}{SST}]}{\partial m} \cdot \frac{\partial SSE}{\partial m} = 0 -  \frac{\partial [\frac{1}{SST}]}{\partial m} \cdot \frac{\partial SSE}{\partial b} $ 
\end{large}
\end{center}
 
Da diese Gleichung = 0 gesetzt wird und $ \frac{\partial [\frac{1}{SST}]}{\partial m} = const. $ ist, kann diese Ableitung vernachlässigt werden. Somit erhält man: \\ 

\begin{center}
\begin{large} 
$ 0 = \frac{\partial SSE}{\partial m} = \frac{\partial [\sum\limits_{i=1}^n [(y_i - b -m \cdot x_i)^2]}{\partial m} = 2 \cdot \sum(y_i \cdot x_i - b \cdot x_i - m \cdot (x_i)^2) $
\end{large}
\end{center}
...
\begin{center}
$ 0 = \sum(y_i \cdot x_i) - b \cdot n \cdot \bar{x} - m \cdot \sum((x_i)^2) \Rightarrow   \sum(y_i \cdot x_i) =  b \cdot n \cdot \bar{x} + m \cdot \sum((x_i)^2) $  (**) 
\end{center}

Schreiben wir nun die zwei Gleichungen mithilfe einer Matrix so erhalten wir:

{\color{red}{***********************************************************************\\ 
Umsetzen der obigen Formel in Matrixschreibweise mit der folgenden Folien ..\\
************************************************************************}}\\[0.2cm]
...
\textbf{Zusammenfassung:}Die Formeln für die Berechnung der Koeffizienten $b$ und $m$ lauten wie folgt:

\begin{center}
\begin{Large}
(I):  $    m = \frac{\sum((x - \bar{x}) \cdot (y - \bar{y}))}{\sum((x - \bar{x})^2)}$   
\\[0.3cm]
(II):     $   b = \bar{y} - m \cdot \bar{x} $                                      
\end{Large}
\\[0.3cm]
\end{center}
...
\begin{large}
Dabei steht:\\
$ \sum $ \text{ für die Summe über alle n Datenpunkte} := $ \sum\limits_{i=1}^n x_i $,\\
$ \bar{x} $ \text{ für den Durchschnittswert der x-Werte} := $ \frac{1}{n} \cdot {\sum\limits_{i=1}^n x_i} $, \\
$ \bar{y} $ \text{ für den Durchschnittswert der y-Werte} := $ \frac{1}{n} \cdot {\sum\limits_{i=1}^n y_i} $, \\
\end{large}

{\color{red}{***********************************************************************\\ 
Umschreibung von Formel (I) und (II) mit der folgenden Folie ..\\
************************************************************************}}\\[0.2cm]
...
\begin{center} 
\hspace*{-2.2cm}
\includegraphics[width=1.3\textwidth]{sLR-LSF-Umschreibung}
\end{center}
