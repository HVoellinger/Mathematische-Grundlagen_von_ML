...
\begin{center}
$ 0 = n \cdot \bar{y} - n \cdot b -n \cdot m \cdot \bar{x} \quad \Rightarrow \quad 0 = \bar{y} - b  -m \cdot \bar{x}$ \\[0.2cm] 
$ \quad \Rightarrow \quad b = \bar{y} -m \cdot \bar{x} \quad\ \Rightarrow \quad \bar{y} = b +  m \cdot \bar{x} \quad\quad\quad $ (1)
\end{center}
...
\begin{center}
\begin{large} 
$ 0 = \frac{\partial (SSE)}{\partial m } = \frac{\partial}{\partial m} \dot  [\sum\limits_{i=1}^n [(y_i - b -m \cdot x_i^2] = -2 \cdot \sum[y_i \cdot x_i - b \cdot x_i - m \cdot x_i^2)] $
\end{large}
\end{center}
...
\begin{center}
$ 0 = \sum(y_i \cdot x_i) - b \cdot n \cdot \bar{x} - m \cdot \sum(x_i^2) $ \\[0.4cm]  
$ \quad\quad\quad \Rightarrow \quad  \sum(y_i \cdot x_i) =  b \cdot n \cdot \bar{x} + m \cdot \sum(x_i^2)\quad\quad  $ (2) \\[0.3cm]
\end{center}
...
\begin{center}
\begin{large} 
$ \begin{bmatrix} 1 & \bar{x} \\ n \cdot \bar{x} & \sum{x_i^2} \\ \end{bmatrix} \cdot                                                                                                  \begin{bmatrix} b \\ m \\ \end{bmatrix} = \begin{bmatrix} \bar{y} \\ \sum{x_i \cdot y_i}  \\ \end{bmatrix} $
\end{large}
\end{center}
...
$ \quad\quad\quad\quad  \begin{bmatrix} b \\ m \\ \end{bmatrix} = \frac{1}{det} \cdot { \begin{bmatrix} \sum{x_i^2} & -\bar{x} \\ -n \cdot \bar{x} & 1 \\ \end{bmatrix} \cdot                                                                                                     \begin{bmatrix} \bar{y} \\ \sum{x_i \cdot y_i} \\ \end{bmatrix}} $ \\[0.8cm]
\end{large}
\end{center} 
Unter Berücksichtigung von ($ det = \sum{x_i^2} - n \cdot \bar{x}^2 $) ist dies äquivalent zu den folgenden zwei Formeln : \\
\begin{center}
\begin{Large}
$ \quad b = \frac{1}{det} \cdot (\bar{y} \cdot \sum{x_i^2} - \bar{x} \cdot \sum{x_i \cdot y_i})\quad\quad\ $ (3) \\[0.5cm]
$ \quad m = \frac{1}{det} \cdot (\sum {x_i \cdot y_i} - n \cdot \bar{x} \cdot \bar{y})\quad\quad\quad\quad $    (4) \\[0.5cm]
\end{Large}
\end{center} 
\textbf{Weitere Umformulierungen von (3) und (4):}\\[0.2cm]

Zur Berechnung des Achsenabschnittes ("intercept") $b$ setzt man nun (P4.1-(i)) und (P4.1-(iii)) in Formel (3). Man kürzt anschließend doppelte Komponenten, so erhält  man: \\[0.2cm]
$ b = \frac{1}{det} \cdot (\bar{y} \cdot \sum{x_i^2} - \bar{x} \cdot \sum{x_i \cdot y_i})\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad $                    (nach Formel (3))
\\[0.2cm]
$ \quad\quad = \frac{1}{det} \cdot (\bar{y} \cdot (\sum{(x_i - \bar{x})^2} + n \cdot \bar{x}^2) - \bar{x} \cdot \sum{x_i \cdot y_i})\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad $ (P4.1-(i)) 
\\[0.2cm]
$ \quad = \frac{1}{det} \cdot (\bar{y} \cdot (\sum{(x_i - \bar{x})^2} + n \cdot \bar{x}^2) - \bar{x} \cdot (n \cdot \bar{x} \cdot \bar{y} + \sum{[(x_i - \bar{x}) \cdot (y_i - \bar{y})]} $      (P4.1-(iii))        \\[0.2cm]
$ = \frac{1}{det} \cdot (\bar{y} \cdot (\sum{(x_i - \bar{x})^2)} - \bar{x} \cdot \sum{[(x_i - \bar{x}) \cdot (y_i - \bar{y})]} \quad $                                   (Doppelten Faktor kürzen)
\\[0.2cm]
Analog lässt sich auch die es Steigung ("slope") $m$ aus Formel (4) umformen und man erhält: \\[0.2cm]
$ m = \frac{1}{det} \cdot \sum{[(x_i - \bar{x}) \cdot (y_i - \bar{y})]}$.
\\[0.5cm]
Definieren wir nun $ \hat{x_i} = (x_i - \bar{x}) $ und $ \hat{y_i} = (x_i - \bar{y}) $ so gilt:\\[0.4cm]
\textbf{Korollar (K4.2)}: 
\begin{center}
\begin{large}
 $ \quad\quad det = \sum{\hat{x_i}}^2 \quad\quad\quad\quad\ $ \\[0.2cm] 
\end{large}
\end{center}
\textbf{Beweis}:\\
wir rechnen:\\
...
Sei $ det = \sum{\hat{x_i}}^2 $. Für die Achsenabschnitte $b$ und die Steigung $m$ einer optimalen sLR - Geraden $ y = b +  m \cdot x $ gelten folgende Formeln:\\[0.5cm]
\begin{large}
(sLSF-I): $ \quad\quad b = \frac{1}{det} \cdot (\bar{y} \cdot \sum{(\hat{x_i}^2)} - \bar{x} \cdot  \sum{[\hat{x_i} \cdot \hat{y_i}]}) \quad\quad $   \\[0.3cm] 
(sLSF-I*): $ \quad\quad b = \bar{y} - m \cdot \bar{x} \quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad $ \\[0.3cm]                                   
(sLSF-II): $ \quad\quad m = \frac{1}{det} \cdot {\sum[\hat{x_i} \cdot \hat{y_i}]} \quad\quad\quad\quad\quad\quad\quad $  \\[0.5cm]
\end{large}
Dabei steht:\\
...
$ b = \frac{1}{det} \cdot (\bar{y} \cdot \sum{\hat{x_i}^2} - \bar{x} \cdot  \sum{[\hat{x_i} \cdot \hat{y_i}]})\quad $                (sLSF-I) \\[0.2cm] 
$  = \frac{1}{\sum{\hat{x_i}}^2} \cdot (\bar{y} \cdot \sum{{\hat{x_i}}^2} - \bar{x} \cdot  \sum{[\hat{x_i} \cdot \hat{y_i}]})\quad $ (Einsetzen (K4.2))\\[0.2cm]
$ = \bar{y} + \bar{x} \cdot [\frac{\sum{[\hat{x_i} \cdot \hat{y_i}]}}{\sum{\hat{x_i}}^2}] \quad\quad\quad\quad\quad\quad\quad\quad\quad $ (Kürzen)\\[0.3cm]
$ = \bar{y} + \bar{x} \cdot m \quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad $                                         (sLF-II)\\    q.e.d.
