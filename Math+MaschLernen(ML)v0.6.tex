...
\begin{document}
\pagenumbering{roman} % römische Seitennummerierung für Titelblatt und Inhaltsverzeichni
...
$ \texttt{(D-4.2): Sum of Squares Residuas (SSE) := } \sum\limits_{i=1}^n \Bigl(y_i - f_i\Bigr)^2 $
...
\begin{itemize}
    \item \textbf{SSE (Sum of Squares Residuals)}: SSE steht für die "Summe der quadrierten Residuen" (Sum of Squares Residuals). Die SSE misst die Gesamtmenge der Abweichungen zwischen den beobachteten Datenpunkten \( y_i \) und den vorhergesagten Werten \( f_i \) der Regressionsgeraden. Es repräsentiert die nicht erklärte Variation in den Daten. Wir schreiben SSE dafür um uns von SSR zu unterscheiden.
...
\end{align*}
%
Beachte, dass der mittlere Ausdruck $\sum_{i=1}^{n} (f_i - \bar{y})^2$ der SSR entspricht und der letzte Ausdruck $2\sum_{i=1}^{n} (y_i - \hat{y}_i)(\hat{y}_i - \bar{y})$ (*) durch algebraische Umformungen gleich null wird (da die Residuen und die Abweichungen vom Mittelwert orthogonal zueinander sind).
