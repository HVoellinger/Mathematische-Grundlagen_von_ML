...
\begin{large}
$ \frac{\color{blue}{SSE}}{\color{red}{SST}} $\\[11.8cm]
\end{large}
...
\\[0.5cm]
Definieren wir nun $ \tilde{x_i} = (x_i - \bar{x}) $ und $ \tilde{y_i} = (x_i - \bar{y}) $ so gilt:\\[0.4cm]
%
\textbf{Korollar (K4.2)}: 
\begin{center}
\begin{large}
 $ \qquad det = \sum{\tilde{x_i}}^2 \qquad \qquad $ \\[0.2cm] 
\end{large}
\end{center}
%
\textbf{Beweis}:\\
...
Sei $ det = \sum{\tilde{x_i}}^2 $. Für die Achsenabschnitte $a$ und die Steigung $a$ einer optimalen sLR - Geraden $ y = a +  b \cdot x $ gelten folgende Formeln:\\[0.5cm]
\begin{large}
(sLSF-I): $ \quad a = \frac{1}{det} \cdot (\bar{y} \cdot \sum{(\tilde{x_i}^2)} - \bar{x} \cdot  \sum{[\tilde{x_i} \cdot \tilde{y_i}]}) \quad\quad $   \\[0.3cm] 
(sLSF-I*): $ \quad a = \bar{y} - b \cdot \bar{x} \qquad \qquad \qquad \qquad\qquad \qquad $ \\[0.3cm]                                   
(sLSF-II): $ \quad b = \frac{1}{det} \cdot {\sum[\tilde{x_i} \cdot \tilde{y_i}]} \qquad \qquad \qquad \qquad $  \\[0.5cm]
\end{large}
Dabei steht:\\
...
%\end{center}
\textbf{Beweis:}\\
Der Beweis der Formeln (sLSF-I) und (sLSF-II) wurde durch Berechnungen oben gezeigt. Der Beweis (sLSF-I*) wurde schon in Formel (1) gezeigt. Wir brweisen in jedoch noch einmal direkt durch Einsetzen von $ det = \sum{\tilde{x_i}}^2 $:\\[0.2cm]
$ a = \frac{1}{det} \cdot (\bar{y} \cdot \sum{\tilde{x_i}^2} - \bar{x} \cdot  \sum{[\tilde{x_i} \cdot \tilde{y_i}]})\qquad $                (sLSF-I) \\[0.2cm] 
$  = \frac{1}{\sum{\tilde{x_i}}^2} \cdot (\bar{y} \cdot \sum{{\tilde{x_i}}^2} - \bar{x} \cdot  \sum{[\tilde{x_i} \cdot \tilde{y_i}]})\qquad $ (Einsetzen (K4.2))\\[0.2cm]
$ = \bar{y} + \bar{x} \cdot [\frac{\sum{[\tilde{x_i} \cdot \tilde{y_i}]}}{\sum{\tilde{x_i}}^2}] \qquad \qquad \qquad \qquad \qquad $ (Kürzen)\\[0.3cm]
