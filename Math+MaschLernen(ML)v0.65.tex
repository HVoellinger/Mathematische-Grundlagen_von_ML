...
\begin{center}
Diese Version V0.65 (Datum: 16.9.2023) ist nicht final!!! \\
...
\begin{Large}
$ \qquad \qquad 
a = \bar{z} - b \cdot \bar{x} - c \cdot \bar{y} \qquad \qquad $ (1)\\[0.2cm]  
\end{Large}
%
Nun benutzen wir den Trick, dass wir die Gleichung (1) auch schon in die partiellen Ableitungen nach $b$ und $c$ einsetzen. Damit müssen wir später keine 3x3-Matrix invertieren, sondern nur eine 2x2-Matrix.\\[0.2cm]
\begin{large} 
\hspace*{1.3cm}
$
\begin{array}[t]{lcl}
...
q.e.d.\\[0.2cm] 
%
{\color{red}{***************************** Weitere analoge Aussagen wie bei sLR-Geraden formulieren (i.e. "SST = SSE + SSR?" etc. ... *********************************}}\\[0.5cm] 
%
\subsection{Allgemeine Mathematische Berechnungen zu $mLR(k\geqq 2$}
%
Die Notation ist analog zun den obigen Definitionen bei $adj.R^2$:\\
In einem {\color{blue}{multiplen Regression Problem}} arbeiten wir mit $n$ Datenpaaren $\langle\mathbf{x}^{(i)}, y^{(i)} \rangle \in \mathbb{R}^{k+1} $ 
wobei $\mathbf{x}^{(i)} \in \mathbb{R}^k$ und $y^{(i)} \in \mathbb{R}$ für alle $i \in \{1,\cdots,n\}$.  \\
Die Zahl $k$ ist die Anzahl der unabhängigen {\color{blue}{Features ("Prädiktoren") $ {\mathbf{x}^{(i)}} $ } }, und {\color{blue}{$ y^{(i)}$}} ist die abhängige {\color{blue}{Zielvariable ("target column")}}. Die Datenpaare nennen wir auch {\color{blue}{Trainings-Menge}}. Unser Ziel ist es die folgende Funktion:  
\\[0.2cm]
...
\end{large}
%
Um die Gleichung $ f\bigl(\mathbf{x}^{(i)}\bigr) \approx y^{(i)}$ weiter zu präzisieren, definieren wir den {\color{blue}{gemittelten quadratischen Fehler "mean squared error"}}: 
\begin{equation}
...
Hier wird $\mathbf{w}$ als der {\color{blue}{Gewichtsvektor}} und $b$ als der {\color{blue}{Bias}} bezeichnet. Es zeigt sich, dass die Notation vereinfacht werden kann, wenn wir den $k$-dimensionalen Merkmalsvektor $\mathbf{x}$ zu einem
$k+1$-dimensionalen Vektor $\mathbf{x}'$ so erweitern, dass
\\[0.2cm]
\hspace*{1.3cm}
$x_j' := x_j$ \quad für alle $j\in\{1,\cdots,k\}$ \quad und \quad $x_{k+1}' := 1$.
\\[0.2cm]
In Worten ausgedrückt: Der Vektor $\mathbf{x}'$ ergibt sich aus dem Vektor $\mathbf{x}$ durch Anhängen der Zahl $1$:
\\[0.2cm]
\hspace*{1.3cm}
$\mathbf{x}' = \langle x_1, \cdots, x_k, 1 \rangle^\top$ \quad wobei $\langle x_1, \cdots, x_k \rangle = \mathbf{x}^\top$.
\\[0.2cm]
weiterhin definieren wir:
\\[0.2cm]
\hspace*{1.3cm}
$\mathbf{w}' = \langle w_1, \cdots, w_k, b \rangle^\top$ \quad wobei $\langle w_1, \cdots, x_k \rangle = \mathbf{w}^\top$.
\\[0.2cm]
Dann haben wir
\\[0.2cm]
\hspace*{1.3cm}
...
\\[0.2cm]
Die Verzerrung ("bias") wurde also in den Gewichtsvektor aufgenommen, und zwar auf Kosten des Anhängens der Zahl $1$ am Ende des Eingabevektors.  Da wir diese Vereinfachung nutzen wollen, gehen wir von nun an davon aus, dass die Eingangsvektoren $\mathbf{x}^{(i)}$ alle so erweitert wurden, dass ihre letzte Komponente $1$ ist.  Unter dieser Annahme definieren wir die Funktion $f$ als:
\\[0.2cm]
\hspace*{1.3cm}
...
\begin{equation}
  ...
  \mathtt{MSE}(\mathbf{w}) = \frac{1}{n-1} \cdot \sum\limits_{i=1}^n \Bigl(\bigl(\mathbf{x}^{(i)})^\top \cdot \mathbf{w} - y^{(i)}\Bigr)^2
\end{equation}
\\[0.2cm]
Unser Ziel ist es, die in dieser Gleichung erscheinende Summe als Skalarprodukt eines Vektors mit selbst darzustellen. Zu diesem Zweck definieren wir zunächst den Vektor $\mathbf{y}$ wie folgt:
\\
\hspace*{1.3cm}
$\mathbf{y} := \langle y^{(1)}, \cdots, y^{(n)} \rangle^\top$.
\\[0.2cm]
...
$
\\[0.2cm]
In der Literatur heisst $X$ auch oft {\color{blue}{Feature Matrix}}. So definiert sind die Zeilenvektoren der Matrix $X$ die transponierten Vektoren $\mathbf{x}^{(i)}$. Nun haben wir folgendes:
\\[0.2cm]
\hspace*{1.3cm}
$X \cdot \mathbf{w} - \mathbf{y} = \left(
  ...
  \begin{array}{c}
    ...
    \vdots \\
    \bigl(\mathbf{x}^{(n)}\bigr)^\top \cdot \mathbf{w} - y_n
  \end{array}
  \right)
$
\\[0.2cm]
...
\begin{equation}
  ...
  \mathtt{MSE}(\mathbf{w}) = \frac{1}{n-1} \cdot \bigl(X \cdot \mathbf{w} - \textbf{y}\bigr)^\top \cdot \bigl(X \cdot \mathbf{w} - \textbf{y}\bigr).
\end{equation}
%
\subsubsection{Berechnung des Gradienten zu $\mathtt{MSE}$}
%
Um das minimale  $\mathtt{MSE}(\mathbf{w})$ zu bekommen, sagt uns die Mathematik (Analysis I), müssen wir den "Gradienten" der Funktion $\mathtt{MSE}(\mathbf{w})$ null setzen. Die folgenden Berechnungen dienen dazu. Nützlich dazu ist die Matrizenrechnung aus der Linearen Algebra.\\[0.2cm]
%
...
\\[0.2cm]
d.h. der \href{https://en.wikipedia.org/wiki/Gradient}{Gradient} \index{gradient} von $\mathtt{MSE}(\mathbf{w})$ in Bezug auf $\mathbf{w}$ muss Null sein. \\
...
%
Als nächstes leiten wir die so genannte {\color{blue}{Normalform}} \index{normal equation} für die Lineare Regression ab. Zu diesem Zweck   erweitern wir zuerst das Produkt im der Gleichung (\ref{eq:squared-error-3}):
\\[0.2cm]
\hspace*{-1.1cm}
$
\begin{array}[t]{lcll}
 \mathtt{MSE}(\mathbf{w}) & = & 
  \frac{1}{n-1} \cdot \bigl(X \cdot \mathbf{w} - \textbf{y}\bigr)^\top \cdot \bigl(X \cdot \mathbf{w} - \textbf{y}\bigr) 
 \\[0.5cm]
 & = & 
 \frac{1}{n-1} \cdot \bigl(\mathbf{w}^\top \cdot X^\top - \textbf{y}^\top\bigr) \cdot \bigl(X \cdot \mathbf{w} - \textbf{y}\bigr) 
 & \mbox{da $(A \cdot B)^\top = B^\top \cdot A^\top$}
 \\[0.5cm]
 & = & 
  \frac{1}{n-1} \cdot \bigl(\mathbf{w}^\top \cdot X^\top \cdot X \cdot \mathbf{w} - \textbf{y}^\top \cdot X \cdot \mathbf{w} 
 - \mathbf{w}^\top \cdot X^\top \cdot \mathbf{y}
+ \mathbf{y}^\top \cdot \mathbf{y} \bigr)
 \\[0.5cm]
 & = & 
 \frac{1}{n-1} \cdot \bigl(\mathbf{w}^\top \cdot X^\top \cdot X \cdot \mathbf{w} - 2 \cdot \textbf{y}^\top \cdot X \cdot \mathbf{w} 
 +  \mathbf{y}^\top \cdot \mathbf{y} \bigr)
 & \mbox{da $\mathbf{w}^\top \cdot X^\top \cdot \mathbf{y} = \textbf{y}^\top \cdot X \cdot \mathbf{w}$}
\end{array}
$
\\[0.4cm]
...
\item Das Matrix Produkt $\mathbf{w}^\top \cdot X^\top \cdot \mathbf{y}$ ist eine reelle Zahl $r$. Das Transponierte  $r^\top$ einer reellen Zahl ist selbst wieder eine reelle Zahl $r$, i.e. ~$r^\top = r$ for all $r \in \mathbb{R}$.  Darum gilt:
      \\[0.2cm]
      \hspace*{1.3cm}
      $\mathbf{w}^\top \cdot X^\top \cdot \mathbf{y} = 
...
\begin{equation}
  ...
  \mathtt{MSE}(\mathbf{w}) = \frac{1}{n-1} \cdot \Bigl(\mathbf{w}^\top \cdot \bigl(X^\top \cdot X\bigr) \cdot \mathbf{w} 
...
\\[0.4cm]
\hspace*{1.3cm}
$ \nabla \mathtt{MSE}(\mathbf{w}) = \frac{2}{n-1} \cdot \Bigl(X^\top \cdot X \cdot \mathbf{w} - X^\top \cdot \mathbf{y}\Bigr)$.
\\[0.4cm]
Falls quadratische Fehler $\mathtt{MSE}(\mathbf{w})$ eien Minimum für die Gewichte $\mathbf{w}$ hat, dann gilt
\\[0.4cm]
\hspace*{1.3cm}
$\nabla \mathtt{MSE}(\mathbf{w}) = \mathbf{0}$.
\\[0.4cm]
...
\\[0.4cm]
\hspace*{1.3cm}
$ \frac{2}{n-1} \cdot \Bigl(X^\top \cdot X \cdot \mathbf{w} - X^\top \cdot \mathbf{y}\Bigr) = \mathbf{0}$.
\\[0.4cm]
...
\end{equation}
Diese Gleichung nennt man {\color{blue}{Normalform}}.  \index{normal equation}\\[0.4cm]
%
\hspace*{1.3cm} Insgesamt erhält man das folgende Theorem: \\[0.4cm]
\textbf{Theorem (Th-5.5): "Gewichtsmatrix bei $mLR(k\geqslant 2$)"} \\[0.3cm]
Die Gewichtsmatrx $\mathbf{w}$ bestimmt die Parameter der optimalen $mLR(k\geqslant 2$)- Hyperebenen. Sie lässt sich bestimmen durch die folgende Gleichung:\\[0.2cm]
\begin{center}
\begin{large}
$ \bigl(X^\top \cdot X\bigr) \cdot \mathbf{w} = X^\top \cdot \mathbf{y}$
\end{large}
...
%
\textbf{Bemerkung:}
Obwohl die Matrix $X^\top \cdot X$ oft invertierbar ist, umschreiben wir sie aus numerischen Gründen nicht als:
\\[0.2cm]
\hspace*{4.8cm}
...
%
\textbf{Beweis}:\\
{\color{red}{Einsetzen der Definitionen in die in (Th-5.5) definierte Normalform für den Fall k=2. Dies ist als Homework zu definieren......\\ q.e.d.}}. \\    
