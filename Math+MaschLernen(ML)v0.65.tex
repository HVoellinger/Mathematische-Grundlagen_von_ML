...
\includegraphics{LSF-mLR(k=2)-3}\\

*** Weitere Kapitel Texte ***

\subsubsection{Weitere Folgerungen für optimale mLR(k=2) Ebene}  
%
{\color{red}{***********************************************************************\\ 
********************** Details analog Vorlesungsfolien für (k=2)*********\\
************************************************************************}}\\[0.2cm]
...
\\[0.2cm]
\hspace*{1.3cm}
$ f(\mathbf{x}) = \mathbf{w} \cdot \mathbf{x} + b = \mathbf{x}' \cdot \mathbf{w}'$.
\\[0.2cm]
...
%
Als nächstes leiten wir die so genannte {\color{blue}{Normalform}} \index{normal equation} for die Lineare Regression ab. Zu diesem Zweck   erweitern wir zuerst da sprodukt im der Gleicung (\ref{eq:squared-error-3}):
\\[0.2cm]
\hspace*{0.3cm}
$
\begin{array}[t]{lcll}
 ...
 \\[0.5cm]
 & = & 
  \frac{1}{m-1} \cdot \bigl(\mathbf{w}^\top \cdot X^\top \cdot X \cdot \mathbf{w} - \textbf{y}^\top \cdot X \cdot \mathbf{w} 
 - \mathbf{w}^\top \cdot X^\top \cdot \mathbf{y}
+ \mathbf{y}^\top \cdot \mathbf{y} \bigr)
 \\[0.5cm]
 & = & 
 \frac{1}{m-1} \cdot \bigl(\mathbf{w}^\top \cdot X^\top \cdot X \cdot \mathbf{w} - 2 \cdot \textbf{y}^\top \cdot X \cdot \mathbf{w} 
 +  \mathbf{y}^\top \cdot \mathbf{y} \bigr)
...
\\[0.2cm]
mag auf den ersten Blick nicht offensichtlich sein. Aber diese folgt  asu zwei Fakten:
\begin{enumerate}
\item Für zwei Matrizen $A$ and $B$ wo das Matrix Produkt $A \cdot B$ is definiert ist , gilt  
      \\[0.2cm]
      \hspace*{1.3cm}
      $(A \cdot B)^\top = B^\top \cdot A^\top$.
\item Das Matrix Produkt $\mathbf{w}^\top \cdot X^\top \cdot \mathbf{y}$ ist eine reelle Zahl. Das Transponierte  $r^\top$ einer reellen Zahl ist selbst wieder eine reelle Zahl $r$, i.e.~$r^\top = r$ for all $r \in \mathbb{R}$.  Darum gilt:
      \\[0.2cm]
      \hspace*{1.3cm}
      $\mathbf{w}^\top \cdot X^\top \cdot \mathbf{y} = 
...
\\[0.2cm]
Dies kann umgeschhrieben werden zu 
\begin{equation}
  \label{eq:normal-equation}
 \colorbox{blue}{\framebox{\colorbox{gray!40}{\framebox{
 $ \bigl(X^\top \cdot X\bigr) \cdot \mathbf{w} = X^\top \cdot \mathbf{y}$}}}} 
\end{equation}
Diese Gleichung nennt man {\color{blue}{Normalform}}.  \index{normal equation}\\
%
Insgesamt erhält man das e Theorem: \\[0.5cm]
