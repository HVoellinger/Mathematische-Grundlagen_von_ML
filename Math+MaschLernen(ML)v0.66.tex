...
\begin{center}
Diese Version V0.66 (Datum: 18.9.2023) ist nicht final!!! \\
...
\\
Dies führte in einigen Fällen auch zu Abbruch des "Data Science" Studienganges. Meine beruflichen Erfahrungen im Bereich der Projektberatung für Data Warehouse Projekte bestätigen dies.\\
...
\end{equation}
Lösen wir die Gleichung nach $b$ auf, nutzen $\bar{\mathbf{x}}^\top = \bar{\mathbf{x}}$ und kürzen mit n,  so erhalten wir mit der Matrixschreibweise:
\begin{large}
\begin{equation}
    \label{eq:squared-error-3}
    b = \bar{\mathbf{y}}- \mathbf{w} \cdot \bar{\mathbf{x}}^\top = \bar{\mathbf{y}} - \mathbf{w} \cdot \bar{\mathbf{x}}   
\end{equation}
\end{large}
...
$
\\[0.2cm]
In der Literatur heisst $X$ auch oft {\color{blue}{Feature Matrix}}. So definiert sind die Zeilenvektoren der Matrix $X$ die transponierten Vektoren $\mathbf{\widetilde{x^{(i)}}}$. Nun haben wir folgendes:
\\[0.4
cm]
\hspace*{1.3cm}
...
$
\\[0.2cm]
Wenn wir das Quadrat des Vektors $X \cdot \mathbf{w} - \mathbf{y}$ nehmen, stellen wir fest, dass wir die Gleichung $\left(\ref{eq:squared-error-2} \right)$ wie folgt umschreiben können:\\[0.3cm]
\begin{equation}
...
\begin{equation}
  ...
  \mathtt{SSE}(\mathbf{w}) = \bigl(\mathbf{w}^\top \cdot X^\top \cdot X \cdot \mathbf{w} - 2 \cdot \mathbf{\widetilde{y}}^\top \cdot X \cdot \mathbf{w} 
 +  \mathbf{\widetilde{y}}^\top \cdot \mathbf{\widetilde{y}} \bigr)\end{equation}
...
\begin{center}
\begin{large}
(LSF-I): $ \qquad  b = \mathbf{w}\cdot \bar{\mathbf{x}} - \bar{\mathbf{y}} \qquad \qquad $ \\[0.5cm]
...
%
\textbf{Bemerkung:}
Obwohl die Matrix $X^\top \cdot X$ oft invertierbar ist, lesen wir häufig auch noch die ursprüngliche Gleichung : $ \bigl(X^\top \cdot X\bigr) \cdot \mathbf{w} = X^\top \cdot \mathbf{\widetilde{y}}$ 
\\[0.2cm]
...
%
\textbf{Beweis}:\\
{\color{red}{Einsetzen der Definitionen in die zwei Formeln von (Th-5.5) für beide Fälle. Dies ist als Homework zu definieren...}}.\\ 
Hinweis: .....Berechne als erstes $X^\top \cdot X$ und danach $ X^\top \cdot \mathbf{\widetilde{y}} $.....  \\
%
q.e.d. \\    
